...
---------------------------------------------------------------------------
-- Boot: the one file allowed to know both CC and the four roles
---------------------------------------------------------------------------

--- A machine built from fakes, with a scripted event queue.
local function booted(role, options)
  options = options or {}
  local clock = fakeClock(0)
  local ports = fakePorts(clock)
  ports.state = nil
...
it("a button press sets a goal rather than sending an order", function()
  -- The difference §5 exists to make: a press dropped by a radio is retried by
...
it("a device the server has never heard of is refused, not invented", function()
  -- The whole point of §6 is telling "there is no miner-7" from "we do not know
...
local function fakeApp(id, name, roles, surfaces)
  return {
...
it("a turtle gets the Services page without a turtle version being written", function()
  ...
  expect.equal(#available, 1, "one app on a turtle")
  expect.equal(available[1].manifest.id, "services", "and it is the diagnostic one")
