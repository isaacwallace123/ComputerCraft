...
--- where D009 lives, and every rule Â§7 states is checked here rather than
...
it("depots keep their order, and can be moved within it", function()
  -- Â§7: ordering is the tie-break when two are equally close. A matched pair
...
it("accepts matches on plain substrings, not Lua patterns", function()
  ...
  -- means something. The conventions are the ones `os/turtle/device/ore.lua` already uses.
