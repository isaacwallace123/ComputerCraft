...
local function call(action, fields)
  ...
  ui.clear()
  ui.header("OPERATIONS", ok and "done" or "failed")
...
local function confirmGridChange()
  return not (state and state.plan and state.plan.configured)
    or ui.menu("CHANGE MINE GRID?", { "Continue (clears old grid)", "Cancel" }) == 1
end
...
while true do
  ...
  local mineLabel = plan.configured
      and ("Mine %d,%d  Y%d"):format(plan.centreX, plan.centreZ, plan.surfaceY)
    or "Mine not configured"
  local labels = {
    mineLabel,
    ("Sectors %s  active %s"):format(summary.opened or 0, summary.active or 0),
    "Refresh",
    "Place mine at base GPS",
    "Place mine at coordinates",
    "Set sector size",
    "Set ring count",
    "Set keepout distance",
    "Assign coordinated quarry",
    "Back",
  }
  local choice = ui.menu("OPERATIONS", labels)
  if not choice or choice == 10 then
    break
  ...
  elseif choice == 8 and confirmGridChange() then
